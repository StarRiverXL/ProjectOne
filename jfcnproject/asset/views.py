...
def assetmanagement(request, option, id):
    ...
    pagenumber = int(request.GET.get('pagenumber', 13))      # 每页展示数量，默认5条
    ...
    if option == "add_asset":
        ...
        if request.method == 'POST':
            ...
            return render(request, 'asset/asset_manage_edit.html', {'asset': asset_management})
        ...
        if pagenumber_switch == 1:
            ...
            contacts = paginatorlist(contact_list, page, pagenumber)
            return render(request, 'asset/asset_js_return_page.html', {'asset_management_list': contacts,
                                                                       'page_condition': page_condition})
        logger.info('返回资产管理界面数据到页面')
        return render(request, 'asset/asset.html', {'asset_management_list': contacts,
                                                    'page_condition': page_condition})
